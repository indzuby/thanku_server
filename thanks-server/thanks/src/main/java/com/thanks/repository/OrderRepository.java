...
public interface OrderRepository extends JpaRepository<OrderObject, Long> {
    ...
    @Query(value= "select *,( 6371 * acos( cos( radians(:lon) ) * cos( radians( lat ) ) * cos( radians( lon ) - radians(:lat) ) + sin( radians(:lon) ) * sin( radians( lat ) ) ) ) as distance from buy b left join order_object oo on b.id=oo.id where match_yn=0 having distance <= :distance", nativeQuery = true)
    List<Buy> findNotMatchedBuyByLocation(@Param("lat") Double lat, @Param("lon") Double lon, @Param("distance") Double distance);

    @Query(value= "select *,( 6371 * acos( cos( radians(:lon) ) * cos( radians( lat ) ) * cos( radians( lon ) - radians(:lat) ) + sin( radians(:lon) ) * sin( radians( lat ) ) ) ) as distance from errand e left join order_object oo on e.id=oo.id where match_yn=0 having distance <= :distance ", nativeQuery = true)
    List<Errand> findNotMatchedErrandByLocation(@Param("lat") Double lat, @Param("lon") Double lon, @Param("distance") Double distance);

    @Query(value= "select *,( 6371 * acos( cos( radians(:lon) ) * cos( radians( start_lat ) ) * cos( radians( start_lon ) - radians(:lat) ) + sin( radians(:lon) ) * sin( radians( start_lat ) ) ) ) as distance from quick q left join order_object oo on q.id=oo.id where match_yn=0 having distance <= :distance", nativeQuery = true)
    List<Quick> findNotMatchedQuickByLocation(@Param("lat") Double lat, @Param("lon") Double lon, @Param("distance") Double distance);

    @Query(value= "select *,( 6371 * acos( cos( radians(:lon) ) * cos( radians( lat ) ) * cos( radians( lon ) - radians(:lat) ) + sin( radians(:lon) ) * sin( radians( lat ) ) ) ) as distance from restaurant_order ro left join order_object oo on ro.id=oo.id left join restaurant r on ro.id=r.id where match_yn=0 having distance <= :distance", nativeQuery = true)
